...
router.post("/Create", upload.array("Attachment", 4), async (request: Request, response: Response) => {
    ...
    if (decoded != undefined) {
        ...
        let isAdded = await functions.insert("Tickets", ticket, ['ID', 'Subject']);
        ...
        if (isAdded.success) {
            ...
            let agents = await functions.select("Users", "Email,FullName", "WHERE Role = 'Admin' OR Role = 'Agent'") as Array<any>;

            email.send(agents, isAdded.result[0].Subject, ticketID, "", "Ticket");
